...
identifyDuplicates <- function(data) {
    ...
    # your code here
    duplicate.matrix <- sapply(data, function(var1) sapply(data, function(var2)
                                       identical(var1, var2)))
    duplicate.matrix[upper.tri(duplicate.matrix, diag=T)] <- F
    duplicate.idcs <- which(duplicate.matrix, arr.ind=T)
    
    if (!nrow(duplicate.idcs))
        return(numeric(0))
    else {
        duplicate.idcs <- t(apply(duplicate.idcs, 1, sort))
        idx.order <- order(duplicate.idcs[, 1])
        duplicate.pairs <- as.matrix(duplicate.idcs[idx.order, ])
        return(unname(duplicate.pairs))
    }
}
