...
generateKids <- function(lambda, kappa, parent) {
    ...
    parent.life.length <- parent[['assassination.date']] - parent[['birth.date']]
    n.kids <- rpois(1, lambda*parent.life.length)

    if (n.kids) {

        # If any n.kids is non-zero, generate a dataframe containing the
        # necessary variables for each child. To do this, you will need to
        ...
        # your code here
        kid.birthdays <- runif(n.kids, parent[['birth.date']],
                               parent[['assassination.date']]) 
        life.lengths <- rexp(n.kids, kappa)

        kids <- data.frame(parent.id=rep(parent[['child.id']], n.kids), child.id=1:n.kids,
                       birth.date=kid.birthdays,
                       assassination.date=parent[['assassination.date']]+life.lengths) 
        return(kids)
...
nextGeneration <- function(lambda, kappa, parents) {
    ...
    next.gen <- apply(parents, 1, function(parent) generateKids(lambda,
                                                                kappa, parent))
    ...
    # If <next.gen> is not null, relabel the <child.id> variable so that it
    # ranges from 1 to nrow(next.gen)

    # your code here
    
    if (!is.null(next.gen)) next.gen$child.id <- 1:nrow(next.gen)
...
bAGen <- function(lambda, kappa, max.gen) {
    ...
    # your code here
    generation.list[[1]] <- data.frame(parent.id=0, child.id=1,
                                         birth.date=0,
                                         assassination.date=rexp(1, kappa))
    ...
    # (i.e. the objected generated by "nextGeneration" is NULL), stop
    # simulating.
    for (gen in 2:max.gen) {

        # your code here
        generation.list[[gen]] <- nextGeneration(lambda, kappa,
                                                 generation.list[[gen-1]])

        if (is.null(generation.list[[gen]])) break
    }

    # Remove extra list elements if the family died out. You do not need to
    # add anything here
...
# title should be "B-A Simulation"

plot(density(gensSurvived(sim.1), bw=.35), col='red',
     xlab='generations survived', main='B-A Simulation', xlim=c(0, 11))
lines(density(gensSurvived(sim.2), bw=.35), col='green')
lines(density(gensSurvived(sim.3), bw=.35), col='blue')
legend('topright', c('1', '2', '3'), col=c('red', 'blue', 'green'), lty=1)
