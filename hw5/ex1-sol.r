...
varProportion <- function(pca) {
    ...
    # your code here
    var.prop <- cumsum(pca$sdev^2)/sum(pca$sdev^2)
    return(var.prop)
}
...
reduceData <- function(data, var.level, scale=T, center=T) {
    ...
    n.comp <- sigComponents(data.pca, var.level) # the number of components to use

    if(!n.comp) return(integer(0))

    reduced <- predict(data.pca)[,1:n.comp]
    return(reduced)
}
...
wine.data <- read.csv('wines.csv', header=T)
wine.reduced <- reduceData(wine.data[, -1], 1)

set.seed(47)
wine.clusters.k <- kmeans(wine.reduced, 3)
cluster.labels.k <- wine.clusters.k$cluster

d.mat <- dist(wine.reduced)
wine.clusters.h <- hclust(d.mat)
cluster.labels.h <- cutree(wine.clusters.h, k=3)
plot(wine.clusters.h)
...
plotClusters <- function(data, variables, cluster.labels, ...) {
    ...
    # your code here
    if (length(variables) != 2) stop('len variables exceeds 2')
    if (length(cluster.labels) != nrow(data)) stop('incompatible dimensions')
    n.clusters <- length(unique(cluster.labels))
    cols <- brewer.pal(n.clusters, 'Set1')
    plot(data[, variables], col=cols[cluster.labels], ...)
}
...
par(mfrow=c(3, 3))
var.combinations <- list(c(1, 2), c(2, 3), c(1, 3))

sapply(var.combinations, function(vars) plotClusters(wine.reduced, vars,
                                                     cluster.labels.k,
                                                     main='k-means', pch=20))
sapply(var.combinations, function(vars) plotClusters(wine.reduced, vars,
                                                     cluster.labels.h,
                                                     main='hierarchical', pch=20))
