...
tpr <- function(threshold, predicted, truth) {
    ...
    # your code here
    return(sum(predicted[as.logical(truth)] > threshold) / sum(truth))
}
...
plotROC <- function(predicted, truth, add=F, ...) {
    ...
    tprs <- sapply(threshold.grid, function(thresh) tpr(thresh, predicted,
                                                        truth))
    fprs <- sapply(threshold.grid, function(thresh) fpr(thresh, predicted,
                                                        truth))

    if (add) lines(fprs, tprs, ...)
    else {
        plot(fprs, tprs, type='l', main='ROC curve',
             xlab='fpr', ylab='tpr', xlim=c(0,1), ylim=c(0,1), ...)
        lines(threshold.grid,threshold.grid, lty=2) }
...
names(spam$data)[ncol(spam$data)] <- 'spam'
...
set.seed(47)
train.idcs <- sample(1:nrow(spam$data), 3500)
spam$train <- spam$data[train.idcs, ]
spam$test <- spam$data[-train.idcs, ]
...
set.seed(47)
fit.rp <- rpart(spam~., method='class', data=spam$train)
fit.rf <- randomForest(spam~., data=spam$train, ntree=250)
pred.rp <- predict(fit.rp, spam$test[,-ncol(spam$test)])
pred.rf <- predict(fit.rf, spam$test[,-ncol(spam$test)], type='prob')
...
test.truth <- as.numeric(spam$test[['spam']])-1
rp.output <- plotROC(pred.rp[,2], test.truth, col='blue')
rf.output <- plotROC(pred.rf[,2], test.truth, col='red', add=T)
legend('bottomright', c('rp', 'rf'), col=c('blue', 'red'), lty=1)
...
constrainedFPR <- function(tpr, fpr, tpr.constraint) {
    ...
    good.idcs <- tpr >= tpr.constraint
    return(min(fpr[good.idcs]))
}
...
min.fpr.rp <- constrainedFPR(rp.output$tpr, rp.output$fpr, 0.95)
min.fpr.rf <- constrainedFPR(rf.output$tpr, rf.output$fpr, 0.95)

rp.threshold <- seq(0, 1, by=0.01)[rp.output$fpr==min.fpr.rp]
rf.threshold <- seq(0, 1, by=0.01)[rf.output$fpr==min.fpr.rf] 
