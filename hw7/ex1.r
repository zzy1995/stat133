...
plot.residuals <- function() {
    plot(fitted(fs.lm),residuals(fs.lm))
   	abline(h=0)
}
...
fheight.slope.confidence.interval <- as.vector(confint(fs.lm,'fheight',level=0.95))
...
ÊÊÊÊb <- predict.lm(fs.lm,newdata=data.frame(fheight=new.fheight),interval="confidence")
	return(b[,-1])
}
...
plot.bands <- function() {
    ...
    conf=predict(fs.lm, interval="confidence")
    pre=predict(fs.lm, interval="prediction")
    lines(sort(father.son$fheight), sort(conf[,2]), col='green')
	lines(sort(father.son$fheight), sort(conf[,3]), col='green')
	lines(sort(father.son$fheight), sort(pre[,2]),col='blue')
	lines(sort(father.son$fheight), sort(pre[,3]),col='blue')
...
r.squared <- function(y, y.fitted) {
    return(summary(lm(y.fitted ~ y))$r.squared)
}
