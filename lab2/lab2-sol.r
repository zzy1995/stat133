...
status.means <- apply(prost.data, 1, function(gene) by(gene, names(gene), mean))

status.trim.means <- apply(prost.data, 1, function(gene) by(gene, names(gene),
                                                            mean, trim=0.1))
...
# your code here
cols <- c('blue', 'red')
boxplot(prost.data, col=cols[as.numeric(colnames(prost.data))], pch='.')
...
quantileCutoff <- function(data, q.cutoff, max.low.expr) {
    ...
    n.low.expr <- rowSums(data < cutoff)
    low.expr.idcs <- which(n.low.expr > max.low.expr)
...
tConvert <- function(gene) {
    ...
    stand.devs <- by(gene, names(gene), sd)
...
# your code here
p.vals <- pValConverter(prost.data)
hist(p.vals, freq=F, breaks=seq(0, 1, by=0.05), main="Prostate Data p-values",
     xlab="p-values")
abline(h=1, col='red')
...
FDR <- function(data, alpha) {
    ...
    n.discoveries <- sum(p.vals < alpha)
    expected.discoveries <- alpha * nrow(data)
    fdr <- expected.discoveries / n.discoveries
    return(fdr)
}
