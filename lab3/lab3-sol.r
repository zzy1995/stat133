...
stratifiedTest <- function(data, group.variable, group.cutoff, test.alternative='less') {
    ...
    # your code here
    group.variable.idx <- which(names(data)==group.variable)
    high.idcs <- data[, group.variable.idx] > group.cutoff

    t.outputs <- by(data[, c('bwt', 'smoke')], high.idcs, function(group) {
        test <- t.test(group$bwt[group$smoke==1], group$bwt[group$smoke==0],
               alternative=test.alternative)
        return(list(test$statistic, test$p.value))
    })

    par(mfrow=c(2, 1))
    by(data[, c('bwt', 'smoke')], high.idcs, function(group) {
...
#stratified.test.t1.1 <- unname(unlist(stratifiedTest(babies.data, "height", 62)))
#stratified.test.t2.1 <- unname(unlist(stratifiedTest(babies.data, "gestation", 260)))
#save(stratified.test.t1, stratified.test.t1.1, stratified.test.t2, stratified.test.t2.1, file="lab3-tests2.rda")
